...
async def create_research_plan(
    ...
    try:
        ...
        if ctx.deps.stream_output:
            ctx.deps.stream_output.steps.append(json.dumps({
                "type": "plan_creation",
                "status": "analyzing",
                "message": "Analyzing topic and creating research plan..."
            }))
        ...
        if match:
            ...
            if ctx.deps.stream_output:
                ...
                ctx.deps.stream_output.steps.append(json.dumps({
                    "type": "plan_created",
                    "status": "complete",
                    "message": f"Created plan with {len(todo_list.todo_items)} research tasks",
                    "tasks": task_data,
    ...
    try:
        ...
        if ctx.deps.stream_output:
            ctx.deps.stream_output.steps.append(json.dumps({
                ...
                "task_title": next_item.description,
                "progress": {
                    "current": completed_count + 1,
                    "total": total_tasks,
                    "percent": int(progress_percent)
                },
                "message": f"Working on task {completed_count+1}/{total_tasks}"
            }))
            await _safe_websocket_send(ctx.deps.websocket, ctx.deps.stream_output)
...
async def mark_todo_item_complete(
    ...
    **kwargs
    ...
    - task_id: ID of the task to mark as complete (optional - will use current task if not provided)
    ...
    """
    # Extract parameters with defaults, checking both task_id and item_id for compatibility
    task_id = kwargs.get('task_id', '') or kwargs.get('item_id', '')
    findings = kwargs.get('findings', '')
    knowledge_gaps = kwargs.get('knowledge_gaps', [])
    report_section = kwargs.get('report_section', '')
    
    try:
        # If no task_id provided, use the current task from context
        if not task_id:
            # Load the todo list to get current task
            if ctx.deps.todo_list_path and os.path.exists(ctx.deps.todo_list_path):
                with open(ctx.deps.todo_list_path, "r") as f:
                    todo_data = json.load(f)
                    todo_list = ResearchTodo.model_validate(todo_data)
                    task_id = todo_list.current_item_id or ctx.deps.current_todo_item
            
            if not task_id:
                return "Error: No task_id provided and no current task found. Please specify which task to mark as complete."
        
        logfire.info(f"Marking todo item {task_id} as complete with params: {kwargs.keys()}")
        
        # Update stream with status
        if ctx.deps.stream_output:
            ctx.deps.stream_output.steps.append(f"Research Progress: Completing task {task_id}...")
            await _safe_websocket_send(ctx.deps.websocket, ctx.deps.stream_output)
        ...
        if item_to_complete.completed:
            return f"Task {task_id} is already marked as complete."
            
        # Store findings
        findings_filename = f"findings_{task_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        ...
        with open(findings_path, "w") as f:
            f.write(f"# Findings for Task {task_id}: {item_to_complete.description}\n\n")
            f.write(findings or "")
        ...
        if task_id not in todo_list.completed_items:
            todo_list.completed_items.append(task_id)
            
        # Update last completed item
        todo_list.last_completed_item_id = task_id
            
        # Reset current item if it's the one being completed
        if todo_list.current_item_id == task_id:
            todo_list.current_item_id = None
            ctx.deps.current_todo_item = None
            
        # Store report section if provided
        if report_section:
            todo_list.report_sections[task_id] = report_section
        ...
        with open(os.path.join(ctx.deps.storage_path, "todo.md"), "w") as f:
            ...
            if ctx.deps.stream_output:
                ...
                ctx.deps.stream_output.steps.append(json.dumps({
                    ...
                    "status": "complete",
                    "task_id": task_id,
                    "task_title": item_to_complete.description,
                    "progress": {
                        "current": completed_count,
                        "total": total_tasks,
                        "percent": progress_percent
                    },
                    "findings_summary": findings[:200] + "..." if findings and len(findings) > 200 else findings,
        ...
        # Update stream with error
        if ctx.deps.stream_output:
            ctx.deps.stream_output.steps.append(f"Error: Could not complete task {task_id}")
            await _safe_websocket_send(ctx.deps.websocket, ctx.deps.stream_output)
        ...
        if ctx.deps.stream_output:
            ...
            for line in lines:
                if line.startswith("Title: "):
                    ...
                    current_source = {'title': line[7:].strip()}
                elif line.startswith("URL: "):
                    current_source['url'] = line[5:].strip()
                elif line.startswith("Snippet: "):
                    current_source['snippet'] = line[9:].strip()
            ...
            ctx.deps.stream_output.steps.append(json.dumps({
                ...
                "status": "complete",
                "query": query,
                "sources": sources[:5],  # Send top 5 sources
    ...
    try:
        ...
        if ctx.deps.stream_output:
            ...
            ctx.deps.stream_output.steps.append(json.dumps({
                ...
                "message": f"Extracted content from {domain}"
            }))
            await _safe_websocket_send(ctx.deps.websocket, ctx.deps.stream_output)
...
async def analyze_search_results(
    ...
    try:
        ...
        if ctx.deps.stream_output:
            # Send findings as structured data
            findings_summary = analysis.get("relevant_findings", "")
            if findings_summary:
                ctx.deps.stream_output.steps.append(json.dumps({
                    "type": "findings_discovered",
                    "status": "complete",
                    "findings": findings_summary[:500] + "..." if len(findings_summary) > 500 else findings_summary,
...
async def stream_thoughts(
    ...
    """Stream the agent's internal thoughts and reasoning process in real-time.
    
    Args:
        thoughts: The agent's current thoughts, reasoning, or internal dialogue
        reasoning_type: Type of reasoning (strategy, analysis, decision, uncertainty, connection, meta-cognitive)
        confidence_level: How confident the agent is in this line of thinking (0.0 to 1.0)
    """
    try:
        logfire.info(f"Streaming thoughts: {reasoning_type} - {thoughts[:100]}...")
        
        # Stream thoughts to frontend
        if ctx.deps.stream_output and ctx.deps.websocket:
            thought_message = json.dumps({
                "type": "agent_thoughts",
            ...
            ctx.deps.stream_output.steps.append(thought_message)
            await _safe_websocket_send(ctx.deps.websocket, ctx.deps.stream_output)
        
        # Return acknowledgment to continue the conversation flow
        return f"Thoughts streamed: {reasoning_type}"
        
    except Exception as e:
        logfire.error(f"Error streaming thoughts: {str(e)}")
        return f"Failed to stream thoughts: {str(e)}"
...
async def stream_reasoning_chain(
    ...
    """Stream a complete chain of reasoning with multiple steps leading to a conclusion.
    
    Args:
        reasoning_steps: List of reasoning steps in order
        conclusion: The conclusion reached from this reasoning chain
        confidence_level: Overall confidence in this reasoning chain (0.0 to 1.0)
    """
    try:
        logfire.info(f"Streaming reasoning chain with {len(reasoning_steps)} steps")
        
        # Stream the reasoning chain to frontend
        if ctx.deps.stream_output and ctx.deps.websocket:
            chain_message = json.dumps({
            ...
            ctx.deps.stream_output.steps.append(chain_message)
            await _safe_websocket_send(ctx.deps.websocket, ctx.deps.stream_output)
        
        return f"Reasoning chain streamed with {len(reasoning_steps)} steps"
        
    except Exception as e:
        logfire.error(f"Error streaming reasoning chain: {str(e)}")
        return f"Failed to stream reasoning chain: {str(e)}"
