...
const GoogleMapsView = () => {
   ...
   const fetchServerStatus = async (isUserAction = false) => {
      try {
         const status = await mcpService.getServerDetails("google_maps");
   ...
   const handleSubmit = async () => {
      ...
      setFeedbackType("");
      try {
         const response = await mcpService.toggleServer("google_maps", token);
         ...
         setFeedbackMessage("Failed to enable Google Maps MCP server");
         setFeedbackType("error");
      } finally {
         setIsLoading(false);
      }
   ...
   const handleDisable = async () => {
      ...
      try {
         ...
         setFeedbackMessage("Failed to disable Google Maps MCP server");
         setFeedbackType("error");
      } finally {
         setIsLoading(false);
      }
   ...
   return (
      <div className="p-8 space-y-8">
         <div className="space-y-4">
            <h1 className="text-4xl font-bold text-center">Google Maps MCP</h1>
            <p className="text-gray-400 text-center">
               Configure Google Maps integration for MCP. Enter your API Key to
               enable the service.
            </p>
            {serverStatus && (
               <div
