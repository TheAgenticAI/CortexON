...
const ClaudeView = () => {
   ...
   const fetchServerStatus = async (isUserAction = false) => {
      try {
         const status = await mcpService.getServerDetails("claude");
   ...
   const handleSubmit = async () => {
      ...
      setFeedbackType("");
      try {
         const response = await mcpService.toggleServer("claude", token);
         ...
         setFeedbackMessage("Failed to enable Claude MCP server");
         setFeedbackType("error");
      } finally {
         setIsLoading(false);
      }
   ...
   const handleDisable = async () => {
      ...
      try {
         ...
         setFeedbackMessage("Failed to disable Claude MCP server");
         setFeedbackType("error");
      } finally {
         setIsLoading(false);
      }
   ...
   return (
      <div className="p-8 space-y-8">
         <div className="space-y-4">
            <h1 className="text-4xl font-bold text-center">Claude MCP</h1>
            <p className="text-gray-400 text-center">
               Configure Claude integration for MCP. Enter your Personal Access
